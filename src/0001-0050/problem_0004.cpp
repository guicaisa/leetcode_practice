...
public:
	...
	// 在A和B中，将A，B分别分为2个数组，A左 + B左构成新数组的左半部分，A右 + B右构成新数组的右半部分，并且两部分的长度相同(或者左半部分比右半部分多一个元素)
	...
	// 
	// 由此结论我们可以推导出一个公式 i + j = m - i + n - j(如果m+n为奇数，这里需要+1)，即j = (m + n + 1) / 2 - i，其中+1是为了处理m+n为奇数时的情况
