...
public:
    //1. 哈希表+滑动窗口
    //建立两个哈希表，t_map用于统计目标字符串t中各字符的出现次数，s_map用于动态记录当前滑动窗口内相关字符的实际出现次数，同时引入count计数器，记录窗口内已达到t_map中字符数量的次数
    //不断向右移动滑动窗口的右边界，将进入窗口并且在t中的字符加入s_map并计数，当s_map中某个字符的数量累加到与t_map中的数量相等时，count自增，表示该字符已完全覆盖
    //当count等于t_map长度时，即当前滑动窗口已包含t的所有字符，开始收缩滑动窗口，同时收缩过程中根据当前滑动窗口大小更新最小覆盖字串长度min_lne和起始索引min_pos
    //然后将滑动窗口左边界向右移动以缩小窗口，如果移出的字符在t中则同步减少其在s_map中的数量，如果移出之后该字符在s_map中的数量小于在t_map中的数量，则count递减，表示该字符已不能覆盖
    //重复此过程，直到滑动窗口不满足覆盖条件之后，再回到向右扩展右边界的阶段，遍历结束后，根据min_len和min_pos处理返回结果
