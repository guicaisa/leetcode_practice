...
public:
    ...
    //循环以每个nums[i]作为起点，在范围[i, nums.size()-1]中查找结果，假设某个索引x，满足nums[i]...nums[x]间所有元素的和大于等于target
    //这个和可以用前缀和来表示，即prefix_sum[x] - prefix_sum[i-1]，所以prefix_sum[x] - prefix_sum[i-1] >= target，可得prefix_sum[x] >= prefix_sum[i-1] + target
    //所以现在问题变成了在范围[i, nums.size()-1]中，查找第一个前缀和大于等于prefix_sum[i-1] + target的值索引，即可获得以nums[i]为起点的最小数组长度
